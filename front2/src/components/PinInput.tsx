...
const PinInput = ({ 
  ...
  return (
    <div
      className={`fixed inset-0 flex flex-col font-hakgyoansim`}
      style={{
        backgroundImage: `
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 pt-12">
        ...
        {pin.length > 0 && (
          <Button
            variant="ghost"
            onClick={handleClear}
            className={`${colors.accent} ${colors.hover} font-hakgyoansim`}
          >
            전체 삭제
          </Button>
        )}
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8">
        ...
        >
          <h1 className="text-2xl font-bold text-gray-800 mb-2 font-hakgyoansim">{title}</h1>
          {subtitle && (
            <p className="text-gray-600 text-lg font-hakgyoansim">{subtitle}</p>
          )}
        </motion.div>
        {/* PIN 입력 표시 */}
            ...
            {[...Array(4)].map((_, i) => (
              <motion.div
                ...
                animate={{ 
                  ...
                  backgroundColor: i < pin.length ? colors.indicatorActive : "#f3f4f6"
                }}
                transition={{ duration: 0.2 }}
                className={`w-8 h-8 rounded-full border-2 ${colors.indicatorBorder} shadow font-hakgyoansim`}
              />
            ))}
          </div>
        </div>
              ...
              {row.map((num, colIdx) => (
                <button
                  ...
                  className={`w-16 h-16 flex items-center justify-center rounded-full text-2xl font-bold font-hakgyoansim bg-white/80 shadow transition-all duration-200
                    ${num && num !== "⌫" ? `${colors.buttonHover}` : ""}
                    ${num === "⌫" ? `${colors.deleteHover}` : ""}
